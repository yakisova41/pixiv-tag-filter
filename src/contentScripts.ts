import { setupConfigMessageReciever } from "./configOperator";
import handleSearchPage from "./handleSearchPage";
import { observeChangeContent, PageChangeEvent } from "./pageChangeObserver";
import {} from "crx-monkey-next/client";
console.log("Pixiv tag filter running");

setupConfigMessageReciever();
observeChangeContent();
...
document.addEventListener(
  ...
  (e: CustomEvent<PageChangeEvent>) => {
    ...
    if (split[1] === "tags" || split[1] === "search") {
      handleSearchPage(e.detail);
    }
  },
);

// If userjs, append setting toggle
if (document.currentScript !== null) {
  const toggle = document.createElement("button");
  toggle.id = "pixiv-tag-filter-toggle";
  toggle.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24">
  ...
  toggle.addEventListener("click", () => {
    window.__crxm__popup[__crxm_build_id]();
  });
}
